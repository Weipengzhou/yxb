...
(function() {
    ...
    [Element, Window, Document].invoke('implement', {
        addEvent: function(type, fn) {
            ...
            var fnWatched = window.CBG_JS_REPORT ? CBG_JS_REPORT.watch(fn) : fn;
            var defn = function() {
                return fnWatched.call(self);
            };
            ...
            function datasetCombineWithAttr(elem) {
                ...
                for (var i = 0, max = attributes.length; i < max; i++) {
                    ...
                    if (name.indexOf('data-') == 0) {
                        var key = toCamelCase(name.split('data-')[1]),
                            value = attr.value;
                    ...
                    Object.keys(css).each(function(key) {
                        ...
                        if ($type(beginValue) === 'array') {
                            ...
                            beginValue.each(function(_beginValue, i) {
                                ...
                                val.push(fn(0, index * BaseTime, _beginValue, _endValue - _beginValue, ctx.duration));
                            });
...
(function() {
    ...
    Request.JSON = new Class({
        ...
        initialize: function(options) {
            ...
            this.addEvent('error', function(text, error) {
                try {
                    ...
                    if (info.length > 100) {
                        info = info.length + '_' +
                            info.substring(0, 20) + '_' +
                            info.replace(/<style[\s\S]*?<\/style>/gi, '').replace(/<script[\s\S]*?<\/script>/gi, '').replace(/<!--[\s\S]*?-->/g, '').replace(/<\/?[^>]*>/g, '').replace(/[\n\r\s  ]/g, '').replace(/&nbsp;/gi, '').substring(0, 80);
                    }
                    sendLog({
        ...
        onSuccess: function(json, text) {
            ...
            watch(this.parent, arguments).call(this);
        },
